...
func MakeRiver(region *m.Region) {
	...
	riverWidth = 3 + rand.Intn(3)
	//fromDirection = rand.Intn(3)
	//toDirection = rand.Intn(3)

	fromDirection = m.DirectionNorth
	toDirection = m.DirectionEast
	...
	if fromDirection == m.DirectionNorth && toDirection != m.DirectionSouth ||
		...
		inflexionsCount = 1
		//inflexionPoint = inflexion{20 + rand.Intn(c.RegionWidth-40), 20 + rand.Intn(c.RegionHeight-40), 0, 0}
		inflexionPoint = inflexion{225, 60, 0, 0}
	} else {
		inflexionsCount = 0
	}
	...
	if inflexionsCount == 0 {
		buildRiverSection(region, startCoord, fromDirection, outPos, inPos, fromBorderLength, riverWidth)
	}
	if inflexionsCount == 1 {
		...
		buildRiverSection(region, startCoord, fromDirection, intermediateOutPos, inPos, inflexionPointDistance, riverWidth)
		...
		buildRiverSection(region, coord{inflexionPoint.x + shiftBeforeturnX, inflexionPoint.y + shiftBeforeturnY}, oppositeDirection, outPos, intermediateInPos, remainingDistance, riverWidth)
	}

}

func buildRiverSection(region *m.Region, startCoord coord, fromDirection, targetPos, inPos, sectionLength, riverWidth int) {
	var shiftsDone = 0
	var padding = 0
	...
	for l := 0; l < sectionLength; l++ {
		...
		for w := start; w < end; w++ {
			switch fromDirection {
			case m.DirectionNorth:
				fillTile(region, coord{startCoord.x + w + shiftVal, startCoord.y + l})
			case m.DirectionSouth:
				fillTile(region, coord{startCoord.x + w + shiftVal, startCoord.y - l})
			case m.DirectionWest:
				fillTile(region, coord{startCoord.x + l, startCoord.y + w + shiftVal})
			case m.DirectionEast:
				fillTile(region, coord{startCoord.x - l, startCoord.y + w + shiftVal})
			}
		}
...
func fillTile(region *m.Region, pos coord) {
	if pos.x >= 0 && pos.y >= 0 && pos.x < c.RegionWidth && pos.y < c.RegionHeight {
		region.Tiles[0][pos.x][pos.y].Surface = m.SurfaceWater
	}
}
